...
int main(int argc, char *argv[])
{
	...
	RSA = r1;
	char prkey ="rsaprivatekey.pem";
	BIO bprkey;
	BIO publ;
        bprkey = BIO_new_file(prkey, "r");
	r1 = PEM_read_bio_RSAPrivateKey( bprkey, NULL, 0, NULL);
	...
	BIO_free_all(hash);
	string k;
	k = PEM_read_bio_PrivateKey(bin, NULL, 0, "hello");
	if (key == NULL)
       		{
       		 cout << "error" << endl;
       		}	
